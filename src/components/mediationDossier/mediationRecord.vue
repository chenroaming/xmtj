<template>
  <div style="padding:20px">
    <div class="form">
      ...
      <div class="about">
        <p class="about-title">
          <span v-if="showAction.id == 9">人民解纷委员会已将人民解纷的相关规定告知各方当事人<br></span>
          {{ showAction.id == 7 ? '调查' : '解纷' }}记录：
        </p>
      ...
      <div v-if="showAction.id == 9" class="dispute">
        <span class="dispute-title">解纷结果：</span>
        <span v-show="!editFlag">{{ formData.results ? formData.results : '未选择' }}</span>
        <el-select v-show="editFlag" v-model="formData.results" placeholder="请选择解纷结果">
          <el-option
