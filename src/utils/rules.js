import { validIDCard } from './validate'
  ...
  data() {
    return {
      rules: {
        ...
        litigantPhone: [
          { required: true, message: '请输入号码', trigger: ['change', 'blur'] }
        ],
        identityCard: [
          {
            required: true,
      ...
      eviRules: {
        litigantId: [{ required: true, message: '请选择当事人', trigger: ['change', 'blur'] }],
        eviName: [{ required: true, message: '请输入证据名称', trigger: ['change', 'blur'] }],
        provideDate: [{ required: true, message: '请选择提供时间', trigger: ['change', 'blur'] }]
